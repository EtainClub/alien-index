...
export async function analyzePrivateImage(buffer: Buffer): Promise<PhotoSignal> {
  const source = sharp(buffer, { failOn: "warning", limitInputPixels: maxPixels });
  ...
  if (!metadata.width || !metadata.height || metadata.width * metadata.height > maxPixels) {
    throw new Error("image-pixel-limit");
  }
  ...
  const dimensionConfidence = Math.min(1, Math.sqrt(buffer.byteLength / 300_000));
  ...
  const qualityLabel = mean < 25 ? "빛이 적은 심우주 톤"
    : mean > 238 ? "밝은 광자 톤"
    : contrast > 55 ? "선명한 이온 대비"
    : "차분한 성운 톤";
